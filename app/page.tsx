...
export default function Home() {
  ...
  const stats = useLive(async () => { const r = await getStats(); if (r.kind !== "AVAILABLE") throw new Error(r.kind === "UNAVAILABLE" ? r.message : "Record not found"); return r.value; }, []);
  ...
  return <main className="page collection-page">
    <section className="reading-room-heading">
      ...
      <div className="ledger-stats" aria-label="Live contract totals"><span><strong>{stats.data?.archive_count ?? "—"}</strong>archives</span><span><strong>{stats.data?.record_count ?? "—"}</strong>records</span><span><strong>{stats.data?.cluster_count ?? "—"}</strong>entities</span><span><strong>{stats.data?.correction_count ?? "—"}</strong>corrections</span></div>
    </section>
