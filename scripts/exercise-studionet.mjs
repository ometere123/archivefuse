...
for (const item of manifest.currentTransactions || (!manifest.currentArchive ? (manifest.liveTransactions || manifest.transactions || []) : [])) if (item.hash) verifiedTransactions.push({ ...item, ...(await verifyTransaction(item.hash)) });

const stats = await client.readContract({ address, functionName: "stats", args: [] });
const archiveEntries = [manifest.currentArchive, ...(!manifest.currentArchive ? [manifest.liveArchive2, manifest.liveArchive] : [])].filter(Boolean);
const archives = [];
...
const records = [];
for (const expected of manifest.currentRecords || (!manifest.currentArchive ? (manifest.liveRecords || manifest.records || []) : [])) {
...
const vec = manifest.currentVecdbProof || (!manifest.currentArchive ? (manifest.liveVecdbProof || manifest.vecdbProof) : null);
let candidates = [];
if (vec?.mode === "proposal_frozen_context") {
  const expectedResolution = (manifest.currentResolutions || manifest.liveResolutions || []).find((x) => x.state.record_a === vec.recordId);
...
for (const expected of manifest.currentResolutions || (!manifest.currentArchive ? (manifest.liveResolutions || manifest.resolutions || []) : [])) {
...
const curator = manifest.currentCuratorProof || (!manifest.currentArchive ? manifest.liveCuratorProof : null);
