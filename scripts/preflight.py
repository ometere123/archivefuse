...
config=(ROOT/'lib'/'genlayer'/'config.ts').read_text(encoding='utf-8'); check('frontend_hardened_schema','required-methods.json' in config and required <= set(json.loads((ROOT/'lib'/'genlayer'/'required-methods.json').read_text(encoding='utf-8'))))
