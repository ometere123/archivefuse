...
## Evidence boundary
All authoritative public sources are HTTPS + SHA-256 bound. The contract rejects malformed/hostless URL authorities, but does not claim a complete SSRF firewall because GenVM does not provide a dependable universal network-policy primitive. Validators independently fetch sources. Non-200, empty, oversized, unavailable or digest-mismatched source bodies fail closed. Fetched text, record metadata and candidate context are explicitly treated as hostile evidence, never instructions. Only a deterministic 7,000-character normalized head/tail excerpt reaches the semantic prompt; decisive evidence should therefore be positioned within that bounded window.

## Candidate and proposal boundaries
VecDB uses the current GenVM global bounded `knn(query, k)` API. ArchiveFuse scans at most `MAX_KNN_SCAN` entries, then filters by archive and entity type; the API does not currently expose a safe server-side filter or offset. Candidate retrieval is consequently bounded and can be crowded by unrelated vectors, but it cannot settle identity. Resolution and correction proposals are intentionally permissionless, with duplicate-pending guards and deterministic stale closure. This is an explicit open-network spam trade-off: temporary pair/membership locks are possible, but no privileged backend, payment or token gate is introduced.

## Invariants
Original records never mutate or delete. Same-entity requires same entity type and at least two shared anchor categories. A record has at most one current cluster. Active membership is a unique projection of retained historical cluster IDs: active IDs are unique, each maps back to that cluster, and `member_count` equals the active list length. Detach clears only current membership; reattach restores it without duplicating historical IDs. Similarity is retrieval only. Person records must respect archive cutoff. Cluster membership is bounded. Superseded clusters report zero active members. Missing or digest-mismatched evidence fails closed. Public list views never create storage as a side effect.
